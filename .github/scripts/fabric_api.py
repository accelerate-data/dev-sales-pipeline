...
def get_powerbi_token() -> str:
    """Get a Power BI access token from the Azure CLI OIDC session.

    api.powerbi.com requires a token scoped to analysis.windows.net/powerbi/api,
    which is a different OAuth audience from api.fabric.microsoft.com.
    No additional Azure permissions are needed — same UAMI, different resource URL.
    """
    result = subprocess.run(
        ["az", "account", "get-access-token",
